...
<div id="content">
	...
	<div id="breadcrumb">
		<a href="<?php echo U('Index/index');?>" title="Go to Home" class="tip-bottom"><i
			class="icon-home"></i> 后台首页</a> <a href="<?php echo U('Arena/arena_game');?>"
			class="current">擂台游戏列表</a> <a href="javascript:;"
			...
			<div class="span12">
				<div class="sousuo">
					<form class="form-horizontal" action="" method="post"
						...
						<table rules="none" border="0">
							<tbody>
								<tr>
									<td class="searchselect"><select name="game_status">
											<option value="">状态</option>
											<option value="1"<?php if($game_status == 1){ echo 'selected';} ?>>启用</option>
											<option value="0"<?php if($game_status == 0){ echo 'selected';} ?>>禁用</option>
											<option value="0"<?php if($game_status == 2){ echo 'selected';} ?>>敬请期待</option>
									</select></td>
									<td class="searchselect"><select name="game_id">
											<option value="">游戏</option>
											<?php if(is_array($games)): $i = 0; $__LIST__ = $games;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["game_id"]); ?>"<?php if($game_id == $vo['game_id']){ echo 'selected';} ?>><?php echo ($vo["game_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
									</select></td>
									<td>
				...
				<div class="widget-box">
					<div class="widget-title">
						...
						<h5>擂台游戏列表</h5>
					 <a href="<?php echo U('Arena/arena_game_edit');?>"><span
					...
					<div class="widget-content">
						<div class="cont_max" id="table_sort">
							<table class="table table-bordered table-striped with-check"
								...
								<thead>
									<tr>
										<th width="50">game_id</th>
										<th width="100">所属游戏</th>
										<th width="50">游戏状态</th>
										<th width="100">擂台游戏图片</th>
										<th width="100">擂台游戏背景图</th>
								...
								<tbody>
									<?php if(is_array($gamelist)): foreach($gamelist as $key=>$row): ?><tr>
										<td><?php echo ($row["game_id"]); ?></td>
										<td><?php echo ($row["game_name"]); ?></td>
										<td><?php if($row['game_status']==1){echo "启用";}elseif($row['game_status']==2){echo '敬请期待';}else{echo '关闭';} ?></td>
										<td><?php echo ($row["game_pic"]); ?></td>
										<td><?php echo ($row["bg_pic"]); ?></td>
										
										<td class="taskOptions"><a href="javascript:;"
											onClick="javascript:window.location.href='<?php echo U('Arena/arena_game_edit',array('game_id'=>$row['game_id']));?>';"
