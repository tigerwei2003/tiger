...
<link rel="stylesheet" href="/gloudapi2/Public/static/css/uniform.css" />
		...
		<div id="content">
			...
			<div id="breadcrumb">
				...
				<a href="<?php echo U('Code/rechargecard');?>" class="current">多选卡类别管理</a>
					...
					<div class="span12">
						...
						<div class="widget-box">
							<div class="widget-title">
                                ...
                                <a href="<?php echo U('Codecard/card_mul');?>"><span class="label btn-primary"><i class=" icon-list-alt icon-white"></i> 多选卡分类管理</span></a>
                               
							</div>
							<div class="widget-content nopadding">
								<form class="form-horizontal" method="post" action="<?php echo U('Codecard/card_type_edit');?>" name="basic_validate" id="basic_validate" enctype="multipart/form-data" novalidate >
                                ...
                                <input type="hidden" name="card_pass" value="<?php echo ($row["card_pass"]); ?>" />
                                    <?php if(!$row): ?><div class="control-group">
                                        ...
                                        <div class="controls">
                                            <input type="text" value="123456" name="card_type_name" id="card_type_name" style="float: left;"/>
                                        </div>
                                    </div>
                                    <div class="control-group">
                                        ...
                                        <div class="controls">
                                            <select name="point_type" id="point_type">
                                            	<?php if(is_array($pointtype)): foreach($pointtype as $key=>$row): ?><option value="<?php echo ($row['id']); ?>"><?php echo ($row["name"]); ?></option><?php endforeach; endif; ?>
                                            </select>
                                        </div>
                                    </div><?php endif; ?>
                                    <div class="control-group">
...
<script type="text/javascript">
	$(document).ready(function(e) {
		...
		$('#point_type').change(function(){ 
			...
			var url = "<?php echo U('Codecard/get_type_point');?>"+"&cid="+cid; 
			...
			$.getJSON(url,function(data){ 
				if(data) { 
					...
					for(var i=0;i<data.length;i ++) {
						...
						html += '<input type="checkbox" name="game_id[]" value="'+data[i]['id']+'|'+encodeURIComponent(data[i]['name'])+'">';
						html += '</label>';
