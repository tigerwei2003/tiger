...
<div id="content">
	...
	<div id="breadcrumb">
		<a href="<?php echo U('Index/index');?>" title="Go to Home" class="tip-bottom"><i
			class="icon-home"></i> 后台首页</a> <a href="<?php echo U('Admin/index');?>"
			class="current">管理员列表</a> <a href="javascript:;"
			...
			<div class="span12">
				<div class="sousuo">
					<form class="form-horizontal" action="" method="post"
						...
						<table rules="none" border="0">
							<tbody>
								<tr>
									<td class="searchselect"><select name="roleid">
											<option value="">角色</option>
											<?php if(is_array($role_data)): foreach($role_data as $key=>$row): ?><option value="<?php echo ($row["id"]); ?>"
												<?php if($user_wh['roleid'] == $row['id']): ?>selected<?php endif; ?>
												><?php echo ($row["remark"]); ?>
											</option><?php endforeach; endif; ?>
									</select></td>
									<td style="padding-right: 0" class="searchselect"><input
										type="text" name="username"
										value="<?php if(isset($user_wh['username'])){ echo $user_wh['username'];}else{ echo '用户名';} ?>"
										onBlur="blur_input(this,'用户名')"
										onClick="click_input(this,'用户名')"></td>
									<td style="padding-right: 0" class="searchselect"><input
										type="text" name="nickname"
										value="<?php if(isset($user_wh['nickname'])){ echo $user_wh['nickname'];}else{ echo '使用者';} ?>"
										onBlur="blur_input(this,'使用者')"
										onClick="click_input(this,'使用者')"></td>
									<td>
				...
				<div class="widget-box">
					<div class="widget-title">
						...
						<a href="<?php echo U('Admin/add');?>"><span class="label btn-success"><i
								class="icon-plus icon-white"></i> 新增管理员</span></a>
					</div>
							...
							<table class="table table-bordered table-striped with-check"
								...
								<thead>
									<tr>
										...
										<th>最近登陆时间</th>
										<th>角色</th>
										<th width="40" class="taskOptions">密码</th>
								...
								<tbody>
									<?php if(is_array($list)): foreach($list as $key=>$row): ?><tr>
										...
										<td><?php echo ($row["role_remark"]); ?></td>
										<td class="taskOptions"><a href="javascript:;"
											onClick="javascript:window.location.href='<?php echo U('Admin/password',array('id'=>$row['id']));?>';"
											class="tip-top" data-original-title="Update" title="修改密码"><i
												class="icon-lock"></i></a></td>


										<td class="taskOptions"><a href="javascript:;"
											onClick="javascript:window.location.href='<?php echo U('Admin/edit',array('id'=>$row['id']));?>';"
											class="tip-top" data-original-title="Update" title="修改资料"><i
												class="icon-pencil"></i></a></td>


										<td class="taskOptions"><a href="javascript:;"
											onClick="javascript:ConfirmDel('<?php echo U('Admin/delete',array('id'=>$row['id']));?>');"
											class="tip-top" data-original-title="Delete" title="删除用户"><i
