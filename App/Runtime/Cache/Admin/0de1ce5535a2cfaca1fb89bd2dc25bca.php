...
<div id="content">
	...
	<div id="breadcrumb">
		<a href="<?php echo U('Index/index');?>" title="Go to Home" class="tip-bottom"><i
			class="icon-home"></i> 后台首页</a> <a href="javascript:;" class="current">每日擂台统计</a>
			...
			<div class="span12">
							...
							<tbody>
								<tr>
									<td class="searchselect"><select name="arena_id">
											...
											<?php if(is_array($arenalist)): $i = 0; $__LIST__ = $arenalist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><option value="<?php echo ($data['id']); ?>"
												<?php if($data['id']==$arena_id){echo 'selected';} ?>
												><?php echo ($data['arena_name']); ?>
				...
				<div class="widget-box">
					<div class="widget-title">
						...
						<h5>每日擂台统计明细</h5>
						<?php if(rolemenu('Export/arenachart')): ?><a
							href="<?php echo U('Export/arenachart',array('arena_id'=>$arena_id,'startdate'=>$startdate,'enddate'=>$enddate));?>"><span
							class="label btn-warning"><i
								class=" icon-download-alt icon-white"></i> 导出</span></a><?php endif; ?>
					</div>
							...
							<table class="table table-bordered table-striped with-check"
								...
								<tbody>
									<?php if(is_array($datalist)): foreach($datalist as $key=>$row): ?><tr>
										...
										<td><?php echo ($row["arena_id"]); ?></td>
										<td><?php echo ($row["arena_name"]); ?></td>
										<td><?php echo ($row["daily_user_nums"]); ?></td>
										<td><?php echo ($row["max_concurrent_user"]); ?></td>
										<td><?php echo ($row["daily_consume_coin"]); ?></td>
										<td><?php echo ($row["max_concurrent_user"]); ?></td>
										<td><?php echo ($row["daily_join_queue_nums"]); ?></td>
										<td><?php echo ($row["daily_leave_queue_nums"]); ?></td>
										<td><?php echo ($row["daily_join_queue_failed_nums"]); ?></td>
										<td><?php echo ($row["daily_account_timeout_nums"]); ?></td>
