...
<div id="content">
	...
	<div id="breadcrumb">
		<a href="<?php echo U('Index/index');?>" title="Go to Home" class="tip-bottom"><i
			class="icon-home"></i> 后台首页</a> <a href="<?php echo U('Game/area');?>"
			class="current">区域管理</a> <a href="javascript:;"
			...
			<div class="span12">
				<div class="widget-box">
					<div class="widget-title">
						...
						<a href="<?php echo U('Game/area_edit');?>"><span
							...
							<table class="table table-bordered table-striped with-check"
								...
								<thead>
									<tr>
										...
										<th width="80">状态</th>
										<th width="80">级别</th>
										<th width="200">测速服务器</th>
										<th width="200">备用测速服务器地址</th>
										<th width="80">测速端口</th>
										<th width="200">GSM服务器地址</th>
										<th width="80">GSM端口</th>
										<th width="200">备注</th>
										<th width="100" class="taskOptions">设置渠道白名单</th>
										<th width="40" class="taskOptions">编辑</th>
										<th width="40" class="taskOptions">删除</th>
									</tr>
								</thead>
								<tbody>
									<?php if(is_array($Area)): foreach($Area as $key=>$row): ?><tr>
										<td><?php echo ($row["id"]); ?></td>
										<td><?php echo ($row["name"]); ?></td>
										<td><?php echo ($row["status"]); ?></td>
										<td><?php echo ($row["level"]); ?></td>
										<td><?php echo ($row["speed_test_addr"]); ?></td>
										<td><?php echo ($row["speed_test_addr_backup"]); ?></td>
										<td><?php echo ($row["speed_test_port"]); ?></td>
										<td><?php echo ($row["gsm_addr"]); ?></td>
										<td><?php echo ($row["gsm_port"]); ?></td>
										<td><?php echo ($row["note"]); ?></td>
											<td class="taskOptions"><a href="javascript:;"
											onClick="javascript:window.location.href='<?php echo U('Region/set_pid',array('id'=>$row['id']));?>';"
											class="tip-top" data-original-title="Update" title="Update"><i
												class="icon-pencil"></i></a></td>
										<td class="taskOptions"><a href="javascript:;"
											onClick="javascript:window.location.href='<?php echo U('Game/area_edit',array('id'=>$row['id']));?>';"
											class="tip-top" data-original-title="Update" title="Update"><i
												class="icon-pencil"></i></a></td>
										<td class="taskOptions"><a href="javascript:;"
											onClick="javascript:ConfirmDel('<?php echo U('Delete/area',array('id'=>$row['id']));?>');"
