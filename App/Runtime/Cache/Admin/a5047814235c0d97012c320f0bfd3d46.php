...
<link rel="stylesheet" href="/gloudapi2/Public/static/css/uniform.css" />
		...
		<div id="content">
			...
			<div id="breadcrumb">
				...
				<a href="<?php echo U('System/pid_logo');?>" class="current">渠道LOGO列表</a>
					...
					<div class="span12">  
                            ...
                            <tbody>
                            	<tr>
                                    ...
                                    <select name="pid">
                                        ...
                                        <?php if(is_array($pid_list)): $i = 0; $__LIST__ = $pid_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["pid"]); ?>" <?php if($pid == $vo['pid']){ echo 'selected';} ?>><?php echo ($vo["pid"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                                    </select>
                                    </td>
                                    <td class="searchselect">
                                    <select name="product">
                                        <option value=''>全部产品</option>
										<option value='0' <?php if($product === '0'){ echo 'selected';} ?>>大厅</option>
										<option value='1' <?php if($product === '1'){ echo 'selected';} ?>>擂台</option>
                                    </select>
                                    </td>
                                    <td class="searchselect">
                                    <select name="client_type">
                                        <option value=''>全部平台</option>
										<option value='0' <?php if($client_type === '0'){ echo 'selected';} ?>>android</option>
										<option value='1' <?php if($client_type === '1'){ echo 'selected';} ?>>win32</option>
                                    </select>
                                    </td>
                                    <td>
                        ...
                        <div class="widget-box">
							<div class="widget-title">
								...
								<h5>渠道LOGO列表</h5>
								<a href="<?php echo U('System/pid');?>"><span class="label btn-success"><i class="icon-plus icon-white"></i>渠道管理</span></a>
								<a href="<?php echo U('System/pid_logo_edit');?>"><span class="label btn-success"><i class="icon-plus icon-white"></i>新增渠道LOGO</span></a>
							</div>
							<div class="widget-content">
								...
								<table class="table table-bordered table-striped with-check" id="maxtable" >
                                    ...
                                    <?php if(is_array($list)): foreach($list as $key=>$row): ?><tr>
                                            ...
                                            <td><img src="<?php echo ($row["logo_url"]); ?>" width="100" height="80" /></td>
                                            <td class="taskOptions">
											<a href="javascript:;" onClick="javascript:window.location.href='<?php echo U('System/pid_logo_edit',array('logo_url'=>$row['logo_url']));?>';" class="tip-top" data-original-title="Update" title="Update"><i class="icon-pencil"></i></a>
											</td>
											<td class="taskOptions">
											<a href="javascript:;" onClick="javascript:window.location.href='<?php echo U('System/pid_logo_del',array('id'=>$row['id']));?>';" class="tip-top" data-original-title="Update" title="Update"><i class="icon-remove"></i></a>
											</td>
