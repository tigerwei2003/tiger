...
<div id="content">
	...
	<div id="breadcrumb">
		<a href="<?php echo U('Index/index');?>" title="Go to Home" class="tip-bottom"><i
			class="icon-home"></i> 后台首页</a> <a href="<?php echo U('Record/gamepack');?>"
			class="current">游戏包购买记录</a> <a href="javascript:;"
						...
						<table rules="none" border="0">
							<tbody>
								<tr>
									...
									<td style="padding-right: 0"><input type="text"
										name="packid"
										value="<?php if($packid){ echo $packid;}else{ echo '游戏包ID';} ?>"
										onBlur="blur_input(this,'游戏包ID')"
										onClick="click_input(this,'游戏包ID')"></td>
									<td style="padding-right: 0"><input type="text"
										name="startdate"
										value="<?php if($startdate){ echo $startdate;}else{ echo '开始日期';} ?>"
										onBlur="blur_input(this,'开始日期')"
										onClick="click_input(this,'开始日期')" title="Go to Home">
									</td>
									<td>-</td>
					...
					<div class="widget-content">
						<div class="cont_max">
							<table class="table table-bordered table-striped with-check"
								...
								<tbody>
									<?php if(is_array($gamepacklist)): foreach($gamepacklist as $key=>$row): ?><tr>
										<td><?php echo ($row["account_id"]); ?></td>
										<td><?php echo ($row["gamepack_id"]); ?></td>
										<td><?php echo ($row["pack_name"]); ?></td>
										<td><?php if($row['create_time']): echo (date('Y-m-d
											H:i:s',$row["create_time"])); endif; ?></td>
										<td><?php if($row['deadline_time']): echo (date('Y-m-d
											H:i:s',$row["deadline_time"])); endif; ?></td>
										<td><?php if($row['last_recharge_time']): echo (date('Y-m-d
											H:i:s',$row["last_recharge_time"])); endif; ?></td>
