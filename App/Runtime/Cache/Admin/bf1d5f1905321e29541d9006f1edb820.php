...
<div id="content">
	...
	<div id="breadcrumb">
		<a href="<?php echo U('Index/index');?>" title="Go to Home" class="tip-bottom"><i
			class="icon-home"></i> 后台首页</a> <a href="<?php echo U('Record/income');?>"
			class="current">虚拟币收入记录</a> <a href="javascript:;"
							...
							<tbody>
								<tr>
									<td style="padding-right: 0"><input type="text"
										name="user"
										value="<?php if($user){ echo $user;}else{ echo '用户';} ?>"
										onBlur="blur_input(this,'用户')"
										onClick="click_input(this,'用户')"></td>
									<td style="padding-right: 0"><select name="income_type">
											<option value="收入类型"
												<?php if($income_type=='收入类型'){echo 'selected';} ?>
												>收入类型
											</option>
											<option value="1"
												<?php if($income_type=='1'){echo 'selected';} ?>
												>签到
											</option>
											<option value="2"
												<?php if($income_type=='2'){echo 'selected';} ?>
												>任务
											</option>
											<option value="3"
												<?php if($income_type=='3'){echo 'selected';} ?>
												>客服赠送
											</option>
											<option value="4"
												<?php if($income_type=='4'){echo 'selected';} ?>
												>购物赠送
											</option>
											<option value="5"
												<?php if($income_type=='5'){echo 'selected';} ?>
												>RMB购买
											</option>
											<option value="6"
												<?php if($income_type=='6'){echo 'selected';} ?>
												>兑换券
											</option>
											<option value="7"
												<?php if($income_type=='7'){echo 'selected';} ?>
												>单款游戏赠送
											</option>
									</select></td>
									<td style="padding-right: 0"><input type="text"
					...
					<div class="widget-content">
						<div class="cont_max">
							<table class="table table-bordered table-striped with-check"
								...
								<tbody>
									<?php if(is_array($incomelist)): foreach($incomelist as $key=>$row): ?><tr>
										<td><?php echo ($row["id"]); ?></td>
										<td><?php echo ($row["account_id"]); ?></td>
										<td><?php echo ($row["bean"]); ?></td>
										<td><?php echo ($row["coin"]); ?></td>
										<td><?php echo ($row["gold"]); ?></td>
										<td><?php if($row['income_type']==1){echo '签到';}elseif($row['income_type']==2){echo '任务';}elseif($row['income_type']==3){echo '客服赠送';}elseif($row['income_type']==4){echo '购物赠送';}elseif($row['income_type']==5){echo 'RMB购买';}elseif($row['income_type']==6){echo '兑换券';}elseif($row['income_type']==7){echo '单款游戏赠送';} ?></td>
