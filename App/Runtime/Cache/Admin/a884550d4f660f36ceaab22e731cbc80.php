...
<div id="content">
	...
	<div id="breadcrumb">
		<a href="<?php echo U('Index/index');?>" title="Go to Home" class="tip-bottom"><i
			class="icon-home"></i> 后台首页</a> <a href="<?php echo U('Record/device');?>"
							...
							<tbody>
								<tr>
									<td><select name="region">
											...
											<?php foreach($regionlist as $v){ if($v['region']){ if($v['region_id']==$region){ $select = 'selected'; }else{ $select = ''; } echo '<option value="'.$v['region_id'].'" '.$select.'>'.$v['region'].'</option>'; } } ?>
									</select></td>
									<td class="searchselect"><select name="isp">
											<option value="">运营商</option>
											<?php foreach($isplist as $v){ if($v['isp']){ if($v['isp_id']==$isp){ $select = 'selected'; }else{ $select = ''; } echo '<option value="'.$v['isp_id'].'" '.$select.'>'.$v['isp'].'</option>'; } } ?>
									</select></td>
									...
									<td class="searchselect"><input type="text" name="account"
										value="<?php if($account){ echo $account;}else{ echo '帐号ID';} ?>"
					...
					<div class="widget-content">
						<div class="cont_max">
							<table class="table table-bordered table-striped with-check"
								...
								<tbody>
									<?php if(is_array($devicelist)): foreach($devicelist as $key=>$row): ?><tr>
										<td><?php echo ($row["id"]); ?></td>
										<td><?php echo ($row["device_uuid"]); ?></td>
										<td><?php echo ($row["model"]); ?></td>
										<td><?php echo ($row["bind_account"]); ?></td>
										<td><?php echo ($row["client_type"]); ?></td>
										<td><?php echo ($row["client_ver"]); ?></td>
										...
										<td><?php if($row['last_login_time']): echo (date('Y-m-d
											H:i:s',$row["last_login_time"])); endif; ?></td>
