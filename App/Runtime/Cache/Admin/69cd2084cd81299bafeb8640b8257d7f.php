...
<link rel="stylesheet" href="/gloudapi2/Public/static/css/uniform.css" />
		...
		<div id="content">
			...
			<div id="breadcrumb">
				...
				<a href="<?php echo U('Checkcardtype/Index');?>" class="current">多选卡类别管理</a>
					...
					<div class="span12">
						...
						<div class="widget-box">
							<div class="widget-title">
                                ...
                                <a href="<?php echo U('Checkcardtype/Index');?>"><span class="label btn-primary"><i class=" icon-list-alt icon-white"></i> 多选卡分类管理</span></a>
							...
							<div class="widget-content nopadding">
                                ...
                                <input type="hidden" name="card_id" value="<?php echo ($typeinfo["id"]); ?>" />
                                <input type="hidden" name="id_string" value="<?php echo ($typeinfo["ids"]); ?>" />
                                    <?php if(!$row): ?><div class="control-group">
                                        <label class="control-label">类别名称</label>
                                        <div class="controls">
                                            <input type="text" value="<?php echo ($typeinfo["type_name"]); ?>" name="card_type_name" id="card_type_name" style="float: left;"/>
                                        </div>
                                    </div>
                                    <div class="control-group">
                                        ...
                                        <div class="controls">
                                        	<?php if(!$typeinfo['id']) {?>
                                            	...
                                            	<?php if(is_array($pointtype)): foreach($pointtype as $key=>$row): ?><option value="<?php echo ($row['id']); ?>" <?php if($typeinfo['point_type']==$row['id']){ echo 'selected';}?>><?php echo ($row["name"]); ?></option><?php endforeach; endif; ?>
                                                </select>
                                                <?php } else { ?>
                                                <select name="point_type" id="point_type">
                                                	<option value="<?php echo ($pointtype['id']); ?>"><?php echo ($pointtype["name"]); ?></option>
                                                </select>
                                ...
                                <div class="controls">
                                	<div class="game_names">
                                        ...
                                        <?php if(is_array($gamepack)): foreach($gamepack as $key=>$row): ?><li><label class="lleft"><input type="checkbox" name="game_id[]" id="game_id[]" value="<?php echo ($row["id"]); ?>" <?php if(in_array($row['id'],$ids)) echo 'checked'; ?>></label> <label class="lright"><?php echo ($row["name"]); ?></label></li><?php endforeach; endif; ?>
                                        </ul>
                                    </div>
                                </div>
                              </div>
                                    <div class="control-group">
                                        ...
                                        <div class="controls">
                                            <input type="text" name="source" value="<?php echo ($typeinfo["spare"]); ?>" id="source"/>
                                        </div>
                                    </div>
                                 <!--   <div class="control-group">
...
<script type="text/javascript">
	$(document).ready(function(e) {
		...
		$('#point_type').change(function(){ 
			...
			var url = "<?php echo U('Checkcardtype/GetTypePoint');?>"+"&cid="+cid; 
