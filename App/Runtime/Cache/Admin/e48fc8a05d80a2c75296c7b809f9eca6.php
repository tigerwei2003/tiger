...
<div id="content">
	...
	<div id="breadcrumb">
		<a href="<?php echo U('Index/index');?>" title="Go to Home" class="tip-bottom"><i
			class="icon-home"></i> 后台首页</a> <a href="<?php echo U('Record/gametimes');?>"
			class="current">游戏时间记录</a> <a href="javascript:;"
				...
				<div class="sousuo">
					<form class="form-horizontal" action="" method="post"
						...
						<table rules="none" border="0">
							<tbody>
								<tr>
									<td class="searchselect"><select name="game">
											<option value="">游戏</option>
											<?php if(is_array($gamelist)): foreach($gamelist as $key=>$gm): ?><option <?php if($game==$gm['game_id']){ echo 'selected';} ?> value="<?php echo ($gm["game_id"]); ?>"><?php echo ($gm["game_id"]); ?>:<?php echo ($gm["game_name"]); ?></option><?php endforeach; endif; ?>
									</select></td>
									<td style="padding-right: 0"><input type="text"
										name="account"
										value="<?php if($account){ echo $account;}else{ echo '账户';} ?>"
										onBlur="blur_input(this,'账户')"
										onClick="click_input(this,'账户')"></td>
									<td style="padding-right: 0"><input type="text"
										name="device"
										value="<?php if($device){ echo $device;}else{ echo '设备';} ?>"
										onBlur="blur_input(this,'设备')"
										onClick="click_input(this,'设备')"></td>
									<td style="padding-right: 0"><input type="text"
										name="gs_ip"
										value="<?php if($gs_ip){ echo $gs_ip;}else{ echo 'GSIP';} ?>"
										onBlur="blur_input(this,'GSIP')"
										onClick="click_input(this,'GSIP')"></td>
									<td style="padding-right: 0"><input type="text"
										name="gs_id"
										value="<?php if($gs_id){ echo $gs_id;}else{ echo 'GSID';} ?>"
										onBlur="blur_input(this,'GSID')"
										onClick="click_input(this,'GSID')"></td>
									<td style="padding-right: 0"><input type="text"
										name="end_code"
										value="<?php if($end_code){ echo $end_code;}else{ echo '结束代码';} ?>"
										onBlur="blur_input(this,'结束代码')"
										onClick="click_input(this,'结束代码')"></td>
									<td style="padding-right: 0"><input type="text"
										name="startdate"
										value="<?php if($startdate){ echo $startdate;}else{ echo '开始日期';} ?>"
										onBlur="blur_input(this,'开始日期')"
										onClick="click_input(this,'开始日期')"></td>
									<td>-</td>
									<td><input type="text" name="enddate"
										value="<?php if($enddate){ echo $enddate;}else{ echo '结束日期';} ?>"
					...
					<div class="widget-content">
						<div class="cont_max">
							<table class="table table-bordered table-striped with-check"
								...
								<tbody>
									<?php if(is_array($gametimes)): foreach($gametimes as $key=>$row): ?><tr>
										<td><?php echo ($row["account_id"]); ?></td>
										<td><?php echo ($row["device_uuid"]); ?></td>
										<td><?php echo ($row["game_name"]); ?></td>
										<td><?php echo ($row["gs_ip"]); ?></td>
										<td><?php echo ($row["gs_id"]); ?></td>
										<td><?php echo ($row["is_online_gs"]); ?></td>
										<td><?php if($row['gs_start_time']): echo (date('Y-m-d
											H:i:s',$row["gs_start_time"])); endif; ?></td>
										<td><?php if($row['gs_last_report_time']): echo (date('Y-m-d
											H:i:s',$row["gs_last_report_time"])); endif; ?></td>
										<td><?php echo ($row["end_code"]); ?></td>
										<td><?php echo ($row['gs_last_report_time'] - $row['gs_start_time']); ?></td>
