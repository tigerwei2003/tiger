...
class DsyUploadFile {//类定义开始
    ...
    private $config =   array(
        ...
        'imageClassPath'    =>  'ORG.Util.DsyImage',    // 图库类包路径
        ...
        'saveRule'          =>  'uniqid',// 上传文件命名规则(如果saveRule属性为空或者0则保持上传文件名不变)
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                for($i=0,$len=count($thumbWidth); $i<$len; $i++) {
                    ...
                    if(1 == $this->thumbType){
                        DsyImage::thumb2($filename,$thumbPath.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true);
                    }else{
                        DsyImage::thumb($filename,$thumbPath.$thumbname.'.'.$thumbExt,'',$thumbWidth[$i],$thumbHeight[$i],true);                        
                    }
    ...
    private function getSaveName($filename) {
        ...
        if(empty($rule)) {//没有定义命名规则，则保持文件名不变
        	...
        	$saveName = md5_file($filename['tmp_name']).".".$filename['extension'];
        }
        if($this->autoSub) {
            // 使用子目录保存文件
